...
@pytest.mark.cupy
def test_liteflownet_model(monkeypatch):
    """Test LiteFlowNet model."""
    shape = (10, 10)
    img1 = np.random.rand(*shape)
    img2 = np.random.rand(*shape)

    from atlinter.optical_flow import LiteFlowNet

    if torch.version.cuda is not None:

        liteflow_model = mock.Mock(
            spec=torch.nn.Module, return_value=torch.rand(1, 2, 10, 10)
        )
        fake_network = mock.Mock()
        fake_network.return_value = liteflow_model
        monkeypatch.setattr("atlinter.optical_flow.LiteFlowNet.Network", fake_network)

        liteflow = LiteFlowNet(device="cpu")
        with pytest.raises(ValueError):
            liteflow.preprocess_images(img1, np.random.rand(20, 20))

        preimg1, preimg2 = liteflow.preprocess_images(img1, img2)
        assert isinstance(preimg1, np.ndarray) and isinstance(preimg2, np.ndarray)
        expected_shape = (1, 3, 32, 32)
        assert (preimg1.shape == expected_shape) and (preimg2.shape == expected_shape)

        flow = liteflow.predict_flow(preimg1, preimg2)
        assert isinstance(flow, np.ndarray)
        assert flow.shape == (2, 10, 10)

        new_img1 = liteflow.warp_image(flow, img2)
        assert isinstance(new_img1, np.ndarray)
        assert new_img1.shape == shape
    else:
        with pytest.raises(AttributeError):
            _ = LiteFlowNet(device="cpu")


@pytest.mark.cupy
def test_pwcnet_model(monkeypatch):
    """Test LiteFlowNet model."""
    shape = (10, 10)
    img1 = np.random.rand(*shape)
    img2 = np.random.rand(*shape)

    from atlinter.optical_flow import PWCNet

    if torch.version.cuda is not None:
        pwc_model = mock.Mock(
            spec=torch.nn.Module, return_value=torch.rand(1, 2, 10, 10).double()
        )
        fake_network = mock.Mock()
        fake_network.return_value = pwc_model
        monkeypatch.setattr("atlinter.optical_flow.PWCNet.Network", fake_network)

        pwc = PWCNet(device="cpu")

        preimg1, preimg2 = pwc.preprocess_images(img1, img2)
        assert isinstance(preimg1, np.ndarray) and isinstance(preimg2, np.ndarray)
        expected_shape = (1, 3, 64, 64)
        assert (preimg1.shape == expected_shape) and (preimg2.shape == expected_shape)

        flow = pwc.predict_flow(preimg1, preimg2)
        assert isinstance(flow, np.ndarray)
        assert flow.shape == (2, 10, 10)

        new_img1 = pwc.warp_image(flow, img2)
        assert isinstance(new_img1, np.ndarray)
        assert new_img1.shape == shape
