...
class LiteFlowNet(OpticalFlow):
    """LiteFlowNet model for optical flow computation.
    ...
    try:
        ...
        sys.argv = ["default", "", "", ""]
        from atlinter.vendor.LiteFlowNet.run import Network
    except ImportError:
        logger.warning("Cuda is not available. It is not possible to run LiteFlowNet")
    ...
    def preprocess_images(self, img1, img2):
        ...
        pad_x = int(((self.shape[1] - 1) // 32 + 1) * 32 - self.shape[1])
        pad_y = int(((self.shape[2] - 1) // 32 + 1) * 32 - self.shape[2])

        img1 = np.pad(img1, ((0, 0), (0, pad_x), (0, pad_y)))
        img2 = np.pad(img2, ((0, 0), (0, pad_x), (0, pad_y)))
    ...
    def predict_flow(self, img1, img2):
        ...
        flow : np.ndarray
            The optical flow.
        """
        img1 = torch.from_numpy(img1).to(self.device).float()
        img2 = torch.from_numpy(img2).to(self.device).float()

        flow = self.model(img1, img2)
        if self.device == "cuda":
            flow = flow.cpu()
        flow = flow.detach().numpy()[0]
        return flow[:, : self.shape[1], : self.shape[2]]


class PWCNet(OpticalFlow):
    """pytorch-pwc-Net model for optical flow computation.
    ...
    try:
        ...
        sys.argv = ["default", "", "", ""]
        from atlinter.vendor.PWC.run import Network
    except ImportError:
        logger.warning("Cuda is not available. It is not possible to run PWCnet")
    ...
    def preprocess_images(self, img1, img2):
        ...
        self.shape = np.array(img1.shape)
        pad_x = int(((self.shape[1] - 1) // 64 + 1) * 64 - self.shape[1])
        pad_y = int(((self.shape[2] - 1) // 64 + 1) * 64 - self.shape[2])
    ...
    def predict_flow(self, img1, img2):
        ...
        flow = 20.0 * self.model(img1, img2)
        if self.device == "cuda":
            flow = flow.cpu()
        flow = flow.detach().numpy()[0]
        return flow[:, : self.shape[1], : self.shape[2]]
